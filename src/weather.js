const API_KEY = "3b3e4388432d1acebb97b817feabc5c5";
const weatherIcon = document.getElementById("weather-icon");
const weatherDetail = document.querySelector("#weather-detail > div");
const weatherDescKo = {
  201: "가벼운 비를 동반한 천둥구름",
  200: "비를 동반한 천둥구름",
  202: "폭우를 동반한 천둥구름",
...
function weatherPrint(data) {
  ...
  const weatherState = weatherDescKo[weatherId];
  const city = data.name;
  const temp = data.main.temp;

  const cityText = document.createElement("span");
  cityText.innerText = `${city} / ${temp}℃`;
  const weatherText = document.createElement("span");
